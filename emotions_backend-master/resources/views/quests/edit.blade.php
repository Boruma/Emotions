...
<body>
  ...
  <div class="row">
    <div class="col-sm-8 offset-sm-2">
      <h1 class="display-3">Quest {{ $quest->id }} bearbeiten</h1>
      ...
      <form method="post" action="{{ route('quests.update', $quest->id) }}">
        ...
        <div class="form-group">
          ...
          <input type="text" class="form-control" name="Name" value="{{ $quest->Name }}" />
        </div>
        ...
        /*
          ...
          fetchPromiseTarget.then(response => {
            ...
            for (let i = 0; i < tragets.length; i++) {
              var option = document.createElement("option");
              option.text = tragets[i].id;
              //Set default option for Select = current assinged point
              if (tragets[i].id == '{{ $quest->target_id }}') {
                option.selected = true;
              }
              x.add(option);
            }
        ...
        <button type="submit" class="btn btn-primary">Quest bearbeiten</button>
      </form>
    </div>
  </div>
</body>
